...
| 10 | Evidence: discovery, clean replay, business-outcome replay, hard-failure replay. Replay writes `evidence/<run_id>/` (events, redacted result, per-step AX + masked PNG, failure state); three replay runs recorded next to the two discovery runs; guardrail fixture (`…risky_fixture.json`) with an unattended-halt run and an escalate-then-abort run | done |
...
| 12 | Stretch: variant-b via tenant overlay. `artifacts/tenants/<id>.json` = base→tenant renames (`renames` for name signals, `frames` for frame titles); `cua replay --tenant <id>` applies it before the run and logs `overlay_applied`. Compiled artifact replays clean on :5001; two evidence runs recorded | done |
